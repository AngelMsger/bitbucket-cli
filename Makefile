...
PKG         := github.com/angelmsger/bitbucket-cli
...
build:
	CGO_ENABLED=0 go build -trimpath -ldflags "$(LDFLAGS)" -o bin/$(BINARY) ./cmd/bitbucket-cli
...
e2e-live: build
	BITBUCKET_E2E_LIVE=1 ./scripts/e2e.sh
...
install-skill:
	mkdir -p $(SKILL_DIR)
	rm -rf $(SKILL_DIR)/bitbucket
	cp -R skills/bitbucket $(SKILL_DIR)/bitbucket
	@echo "installed skill -> $(SKILL_DIR)/bitbucket"
