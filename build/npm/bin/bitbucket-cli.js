#!/usr/bin/env node
'use strict';
// Thin launcher for the bitbucket-cli binary. It execs the platform binary
...
async function main() {
  ...
  if (!fs.existsSync(file)) {
    process.stderr.write('bitbucket-cli: downloading binary...\n');
    await install();
  }
  ...
  if (res.error) {
    process.stderr.write(`bitbucket-cli: ${res.error.message}\n`);
    process.exit(1);
  }
...
main().catch((err) => {
  process.stderr.write(`bitbucket-cli: ${err.message}\n`);
  process.exit(1);
});
