'use strict';
// install.js downloads the prebuilt bitbucket-cli binary that matches the host
...
const REPO = 'AngelMsger/bitbucket-cli';
...
function assetName() {
  ...
  return `bitbucket-cli-${goos}-${goarch}` + (goos === 'windows' ? '.exe' : '');
}

// binPath returns the directory and file path for the installed binary.
function binPath() {
  const dir = path.join(__dirname, 'binary');
  const exe = process.platform === 'win32' ? 'bitbucket-cli.exe' : 'bitbucket-cli';
...
function httpGet(url, redirects = 0) {
  return new Promise((resolve, reject) => {
    ...
    https
      .get(url, { headers: { 'User-Agent': 'bitbucket-cli-npm-installer' } }, (res) => {
...
// interactive run (see bin/bitbucket-cli.js). It points at the two setup
// commands and a couple of everyday ones.
function welcomeText() {
  return [
    '',
    'bitbucket-cli is ready. First-time setup:',
    '',
    '  bitbucket-cli config init --pretty   configure your server + credentials (interactive)',
    '  bitbucket-cli skill install          install the coding-agent Skill',
    '',
    'Everyday use:',
    '  bitbucket-cli pr list --repo <workspace>/<repo>',
    '  bitbucket-cli pr diff <workspace>/<repo>/<id>',
    '  bitbucket-cli --help',
    '',
    'Docs: https://angelmsger.github.io/bitbucket-cli/',
    '',
  ].join('\n');
}
...
if (require.main === module) {
  install()
    .then((file) => {
      process.stdout.write(`bitbucket-cli: installed ${file}\n`);
    })
    .catch((err) => {
      process.stderr.write(
        `bitbucket-cli: postinstall download skipped (${err.message}); ` +
