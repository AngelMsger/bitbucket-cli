...
This repository is a Go CLI for Bitbucket. The executable entrypoint is in `cmd/bitbucket-cli/`. Core implementation lives under `internal/`, grouped by responsibility: `app` for Cobra commands, `apiclient` for Bitbucket APIs, `auth` and `config` for setup, `output` for presentation, and `transport` for HTTP behavior. Public helper packages are in `pkg/`. Tests sit beside the code as `*_test.go`; the mock Bitbucket server is in `test/mockserver/`. Documentation is in `docs/`, companion agent skill files are in `skills/bitbucket/`, and npm wrapper/release assets are in `build/npm/`.
