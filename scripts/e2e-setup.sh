...
cd "$WORK"
run() {
  env -i PATH="$PATH" HOME="$WORK" NO_COLOR=1 BITBUCKET_CLI_NO_UPDATE_NOTIFIER=1 \
...
out="$(env -i PATH="$PATH" HOME="$WORK" NO_COLOR=1 BITBUCKET_CLI_NO_UPDATE_NOTIFIER=1 BITBUCKET_CONTEXT=unconfigured \
  BITBUCKET_TOKEN=do-not-save BITBUCKET_USERNAME=do-not-save \
...
out="$(env -i PATH="$PATH" HOME="$WORK" NO_COLOR=1 BITBUCKET_CLI_NO_UPDATE_NOTIFIER=1 BITBUCKET_SERVER=https://env.invalid/deploy \
  BITBUCKET_AUTH_SCHEME=pat BITBUCKET_CREDENTIAL_URL=https://help.invalid/env \
