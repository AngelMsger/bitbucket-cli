...
func TestCachedMissThenHit(t *testing.T) {
	...
	// First call: cache miss -> one network lookup, writes the cache.
	...
	// Second call: fresh cache -> served without any network lookup.
	st = Cached(context.Background(), doer, dir, "1.0.0")
	if !st.Available || st.Latest != "2.0.0" {
...
func TestCachedNoDirNoCache(t *testing.T) {
	...
	// With no cache dir each call performs a fresh lookup and never persists.
