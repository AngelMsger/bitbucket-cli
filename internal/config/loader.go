...
type LoadOptions struct {
	...
	// over BITBUCKET_CONTEXT and the file's current_context.
	Context string
}
...
// BITBUCKET_CONTEXT env var, the file's current_context, the sole context,
...
func selectContext(f File, flagCtx, envCtx string) (string, string, error) {
	...
	case envCtx != "":
		return pick(envCtx, "BITBUCKET_CONTEXT", ContextSourceEnv)
...
func unknownContextHint(name string, available []string) string {
	...
	return "Run `bitbucket-cli config get-contexts` to list defined contexts."
}
...
func buildFileLayer(f File, ctxName string) map[string]string {
	...
	put(m, fieldWorkspace, f.Defaults.Workspace)
...
func Load(opt LoadOptions) (*Resolved, error) {
	...
	if opt.Setup {
		...
		ctxName, ctxSource, err = selectContext(file, opt.Context, os.Getenv("BITBUCKET_CONTEXT"))
		if err != nil {
			return nil, err
		}
	}
