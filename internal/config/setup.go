...
func Guide(cfg Config, sources map[string]string) (AuthGuide, error) {
	...
	if g.Flavor == FlavorCloud {
		...
		g.Instructions = []string{"Create an API token for this product and use your Atlassian account email as the username.", "For personal login use basic authentication with an API token with Bitbucket scopes; workspace/repository access tokens use pat authentication."}
	} else {
		g.DocumentationURL = "https://confluence.atlassian.com/bitbucketserver/http-access-tokens-939515499.html"
		if g.Flavor == FlavorDataCenter {
			g.CredentialURL = base + "/plugins/servlet/access-tokens/manage"
			g.Source = "builtin"
		}
		g.Instructions = []string{"Open your avatar, then Manage account, then HTTP access tokens. Create a token with the permissions needed for your work.", "Page paths differ by server version; use the navigation steps or set --credential-url if the link does not work."}
	}
