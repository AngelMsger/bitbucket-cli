...
func TestLoadPrecedenceEnvOverridesDotenvAndFile(t *testing.T) {
	...
	writeFile(t, dotenv, "BITBUCKET_SERVER=https://dotenv.example.com\n")
	t.Setenv("BITBUCKET_SERVER", "https://env.example.com")
...
func TestLoadSecretFromEnvInfersScheme(t *testing.T) {
	dir := t.TempDir()
	t.Setenv("BITBUCKET_PERSONAL_ACCESS_TOKEN", "tok-123")
	got, err := Load(LoadOptions{ConfigDir: dir, DotenvPath: filepath.Join(dir, "absent.env")})
	if err != nil {
		t.Fatal(err)
	}
...
func TestDotenvDoesNotMutateProcessEnv(t *testing.T) {
	dir := t.TempDir()
	dotenv := filepath.Join(dir, ".env")
	writeFile(t, dotenv, "BITBUCKET_FORMAT=table\n")
	if _, err := Load(LoadOptions{ConfigDir: dir, DotenvPath: dotenv}); err != nil {
		t.Fatal(err)
	}
	if _, ok := os.LookupEnv("BITBUCKET_FORMAT"); ok {
...
func TestLoadMultiContextSelection(t *testing.T) {
	...
	t.Run("env overrides current_context", func(t *testing.T) {
		dir := t.TempDir()
		writeMultiContextFile(t, dir)
		t.Setenv("BITBUCKET_CONTEXT", "beta")
		got, err := Load(LoadOptions{ConfigDir: dir, DotenvPath: absent(dir)})
		if err != nil {
			t.Fatal(err)
		}
	...
	t.Run("BITBUCKET_SERVER still overrides selected context", func(t *testing.T) {
		dir := t.TempDir()
		writeMultiContextFile(t, dir)
		t.Setenv("BITBUCKET_SERVER", "https://override.example.com")
		got, err := Load(LoadOptions{ConfigDir: dir, DotenvPath: absent(dir)})
		if err != nil {
			t.Fatal(err)
		}
...
func TestResolveConfigDir(t *testing.T) {
	...
	t.Run("neither exists → new path", func(t *testing.T) {
		...
		want := filepath.Join(home, ".angelmsger", "bitbucket")
		if dir != want {
	...
	t.Run("only legacy has config.yaml → legacy path", func(t *testing.T) {
		home := withFakeHome(t)
		legacy := filepath.Join(home, ".bitbucket")
	...
	t.Run("new has config.yaml → new path wins", func(t *testing.T) {
		home := withFakeHome(t)
		newDir := filepath.Join(home, ".angelmsger", "bitbucket")
