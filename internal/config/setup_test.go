...
func TestSetupTargetsNamedContextAndExcludesPersonalEnvironment(t *testing.T) {
	...
	t.Setenv("BITBUCKET_CONTEXT", "not-defined")
	t.Setenv("BITBUCKET_FORMAT", "ndjson")
	t.Setenv("BITBUCKET_USERNAME", "InjectedUser")
	t.Setenv("BITBUCKET_API_TOKEN", "must-not-be-copied")
	dotenv := filepath.Join(dir, "presets.env")
	if err := os.WriteFile(dotenv, []byte("BITBUCKET_SERVER=https://dotenv.example.test/deploy\nBITBUCKET_CREDENTIAL_URL=https://help.example.test/new\nBITBUCKET_API_TOKEN=dotenv-secret\nBITBUCKET_USERNAME=dotenv-user\n"), 0600); err != nil {
		t.Fatal(err)
	}
	...
	t.Setenv("BITBUCKET_SERVER", "https://env.example.test")
	t.Setenv("BITBUCKET_AUTH_SCHEME", "basic")
	got, err = Load(opt)
	if err != nil {
		t.Fatal(err)
	}
...
func TestPlanServiceContextConflictIdempotencyAndPreservation(t *testing.T) {
	...
	dir := t.TempDir()
	target := setupFixture()
	file := File{CurrentContext: "other", Contexts: []NamedContext{{Name: "other", BaseURL: "https://other.example.test"}, target}, Defaults: Defaults{ReadOnly: true, Workspace: "KEEP"}}
	if err := WriteFile(dir, file); err != nil {
		t.Fatal(err)
	}
...
func TestCredentialGuideIsDisplayOnlyAndKeepsDeploymentPath(t *testing.T) {
	...
	if g.CredentialURL != nc.BaseURL+"/plugins/servlet/access-tokens/manage" {
