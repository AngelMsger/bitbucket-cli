...
// (~/.angelmsger/bitbucket). Callers that want the smart "prefer new,
...
// LegacyConfigDir returns the pre-0.4 config directory (~/.bitbucket).
...
// supplied. It prefers the new ~/.angelmsger/bitbucket location, but falls
// back to the legacy ~/.bitbucket directory when the new path has no
...
func WriteFile(dir string, f File) error {
	...
	fs.Defaults.Workspace = f.Defaults.Workspace
