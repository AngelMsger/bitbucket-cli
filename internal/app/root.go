...
func newRootCmd() *cobra.Command {
	...
	root := &cobra.Command{
		...
		Short: "Drive Bitbucket repositories and pull requests from coding agents",
	...
	pf.StringVar(&state.gflags.configPath, "config", "", "config directory (default ~/.bitbucket)")
	...
	pf.BoolVar(&state.gflags.allowWrites, "allow-writes", false,
		"override read-only mode (defaults.read_only / BITBUCKET_CLI_READ_ONLY) for this invocation")

	root.SetFlagErrorFunc(func(_ *cobra.Command, err error) error {
		return cerrors.Wrap(err, cerrors.CategoryUsage, "BAD_FLAG", err.Error())
	})
	// `bitbucket-cli --version` prints the same line as the `version` command.
